...
function install_xcode() {
    ...
    if [ -n "${APPLEID_USER-}" ] && [ "${#APPLEID_USER}" -gt "0" ] &&
        ...
        for XCODE_VERSION in "${XCODE_VERSIONS[@]}"; do
            xcodes install --use-fastlane-auth --verbose "$XCODE_VERSION"
        done

        if [ "$OSX_MAJOR_VER" -ge 12 ]; then
            xcodes runtimes install --use-fastlane-auth --verbose 'iOS 15.5'
            xcodes runtimes install --use-fastlane-auth --verbose 'watchOS 8.5'
            xcodes runtimes install --use-fastlane-auth --verbose 'tvOS 15.4'
        fi
